...
module.exports = async (
  ...
  checkForOrphanedSubstitutions(substitutions, flattenedChapters);
    ...
    switch (sigil) {
      ...
      case "@ ": {
        ...
        if (result.has(currentChapter)) {
          throw new Error(`${errorPrefix}duplicate chapter declaration`);
        }
...
function checkForOrphanedSubstitutions(substitutions, chapters) {
  const chapterURLsSet = new Set(chapters.map(chapter => chapter.url));
  for (const substitutionsChapterURL of substitutions.keys()) {
    if (!chapterURLsSet.has(substitutionsChapterURL)) {
      throw new Error(`Orphaned substitution for ${substitutionsChapterURL}`);
    }
  }
}
