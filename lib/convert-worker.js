...
function fixHyphens(xml) {
  ...
  xml = xml.replace(/(well) (armed|stocked|rounded|oiled|made|placed)/vig, "$1-$2");
  ...
  xml = xml.replace(/(long|short) (sleeved|ranged)/vig, "$1-$2");
  ...
  xml = xml.replace(/(fast|slow) (moving|paced)/vig, "$1-$2");
  ...
  xml = xml.replace(/(re)-establish/vig, "$1establish");
  xml = xml.replace(/(non)-sequitur/vig, "$1 sequitur");
  xml = xml.replace(/Middle-Eastern/vig, "Middle Eastern");
  xml = xml.replace(/(flood)-lamp/vig, "$1lamp");
...
function standardizeSpellings(xml) {
  ...
  // 10 instances of "eye shadow" to 7 of "eyeshadow"
