...
function fixCapitalization(xml, bookTitle) {
  ...
  // Similarly, "Director Piggot" is capitalized, but "the director" should not be. (This is a very prevalent mistake.)
  xml = xml.replace(/([Tt]he) Director/vg, "$1 director");
  xml = xml.replace(/([Tt]he) Mayor/vg, "$1 mayor");

  // "the president" is lowercase, but "the President of the United States" is capitalized.
  xml = xml.replace(/([Tt]he) President(?! of the United States)/vig, "$1 president");
  xml = xml.replace(/([Tt]he) Vice President(?! of the United States)/vig, "$1 vice president");
...
function fixHyphens(xml) {
  ...
  xml = xml.replace(/(tinker) (made)/vig, "$1-$2");
