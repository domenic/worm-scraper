...
function fixCapitalization(xml, bookTitle) {
  ...
  // These are missing their capitalization a couple times.
  ...
  xml = xml.replace(/gobling king/vg, "Goblin King");
  ...
  // Proper noun/adjectives sometimes strangely missing capitalization.
  ...
  // Similarly, "yakuza" is like "gangster" or "mafia", not a proper name.
  ...
  // Similarly, "Director Piggot" is capitalized, but "the director" and "directors" should not be. (This is a very
  // prevalent mistake.)
...
function fixMispellings(xml) {
  ...
  // https://www.dictionary.com/browse/eyewitness
  xml = xml.replace(/eye witness/vg, "eyewitness");

  xml = xml.replace(/gasmask/vg, "gas mask");

  xml = xml.replace(/soiree/vg, "soirée");
  ...
  xml = xml.replace(/(S|s)uper-ego/vg, "$1uperego");
...
function fixHyphens(xml) {
  ...
  xml = xml.replace(/(second) (degree) (burns|murder)\b/vig, "$1-$2 $3");
  ...
  xml = xml.replace(/(high) (rise)/vig, "$1-$2");
  ...
  xml = xml.replace(/(heavy) (hitting)/vig, "$1-$2");
  xml = xml.replace(/(post) (traumatic)/vig, "$1-$2");
  xml = xml.replace(/(color) (coded)/vig, "$1-$2");
  xml = xml.replace(/(self) (professed)/vig, "$1-$2");
  xml = xml.replace(/(mass) (produc)/vig, "$1-$2");
  xml = xml.replace(/(second) (degree) (burn)/vig, "$1-$2 $3");
