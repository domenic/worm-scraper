...
async function getChapterInfo(contentPath, chaptersPath, manifestPath, chapterDataPath, chapterTitleStyle) {
  ...
  for (const arc of chapterData) {
    ...
    for (const chapter of arc.chapters) {
      ...
      tocHTML += `          <li><a href="${chapterHref}">${chapterTitle}</a>\n`;
    }
    tocHTML += `        </ol>
      </li>\n`;
...
function augmentAndCheckChapterData(chapterData, manifestChapters) {
  for (const chapter of manifestChapters) {
    let found = false;
    for (const arc of chapterData) {
      for (const chatperInArc of arc.chapters) {
        if (chapter.url === chatperInArc.url) {
          chatperInArc.filename = `${path.basename(chapter.filename, ".html")}.xhtml`;
          found = true;
          break;
        }
      }
    ...
    if (!found) {
      throw new Error(`Chapter data not found for ${chapter.url} which appeared in the manifest`);
    }
  }
