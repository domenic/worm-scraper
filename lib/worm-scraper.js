...
const chapterDataPath = path.resolve(__dirname, "../chapter-data", `${argv.book}.json`);
...
if (argv._.includes("download")) {
  const { startURL, stopURL } = books[argv.book];
  commands.push(() => download(startURL, stopURL, cachePath, manifestPath));
}
...
if (argv._.includes("convert")) {
  commands.push(async () => {
    ...
    await convert(cachePath, manifestPath, chapterDataPath, chaptersPath, argv.book, argv.jobs, argv["chapter-titles"]);
  });
}
