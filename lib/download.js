...
module.exports = async (startChapterURL, stopChapterURL, cachePath, manifestPath) => {
  let manifestContents;
  try {
    manifestContents = await fs.readFile(manifestPath, { encoding: "utf-8" });
  } catch (e) {
    if (e.code === "ENOENT") {
      return downloadAllChapters(null, startChapterURL, stopChapterURL, cachePath, manifestPath);
    }
    throw e;
  }

  const manifest = JSON.parse(manifestContents);
  return downloadAllChapters(manifest, startChapterURL, stopChapterURL, cachePath, manifestPath);
};

async function downloadAllChapters(manifest, startChapterURL, stopChapterURL, cachePath, manifestPath) {
  ...
  while (currentChapter !== null) {
    let breakAfterFinishingThisChapter = false;
    if (currentChapter === stopChapterURL) {
      breakAfterFinishingThisChapter = true;
    }
    ...
    if (breakAfterFinishingThisChapter) {
      break;
    }
